...
function CoverPage({ team, season }: { team: Team; season: string }) {
    ...
    >
      ...
      <div style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "60px 80px", position: "relative" }}>
        ...
        <div style={{
          ...
          background: "rgba(249,115,22,0.08)", marginBottom: 40, flexShrink: 0,
          boxShadow: `0 0 60px rgba(249,115,22,0.25)`,
        }}>
          {team.logoUrl
            ? <img src={team.logoUrl} alt={team.name} style={{ width: "100%", height: "100%", objectFit: "cover" }} crossOrigin="anonymous" />
            : <span style={{ fontSize: 56, fontWeight: 900, color: ORANGE }}>{ini}</span>
          }
        </div>
      ...
      <div style={{
        ...
        <span style={{ color: "#374151", fontSize: 11, fontWeight: 700, letterSpacing: "0.1em", textTransform: "uppercase" }}>
          ScoutPro
        </span>
...
function PlantillaPage({ team, players, season }: { team: Team; players: Player[]; season: string }) {
  const ini = initials(team.name);
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{
          ...
          overflow: "hidden", display: "flex", alignItems: "center", justifyContent: "center",
          background: "rgba(249,115,22,0.06)", flexShrink: 0,
        }}>
          {team.logoUrl
            ? <img src={team.logoUrl} alt={team.name} style={{ width: "100%", height: "100%", objectFit: "cover" }} crossOrigin="anonymous" />
            : <span style={{ fontSize: 18, fontWeight: 900, color: ORANGE }}>{ini}</span>
          }
        </div>
        <div style={{ flex: 1 }}>
          <p style={{ margin: 0, fontSize: 10, fontWeight: 800, color: ORANGE, letterSpacing: "0.2em", textTransform: "uppercase" }}>
            Plantilla
          </p>
          <h2 style={{ margin: "2px 0 0", fontSize: 22, fontWeight: 900, color: "#111827", textTransform: "uppercase", fontStyle: "italic" }}>
            {team.name}
          </h2>
        </div>
        ...
        {players.slice(0, 22).map((p, idx) => {
          ...
          return (
            <div key={p.id} style={{
              ...
              {/* # */}
              <div style={{ width: 32, fontSize: 12, fontWeight: 700, color: "#d1d5db", fontFamily: "monospace" }}>
                {p.jerseyNumber ?? "—"}
              </div>
              {/* Foto + nombre */}
              <div style={{ width: 260, display: "flex", alignItems: "center", gap: 10 }}>
                <div style={{
                  ...
                  {p.photoUrl
                    ? <img src={p.photoUrl} alt={p.name} style={{ width: "100%", height: "100%", objectFit: "cover" }} crossOrigin="anonymous" />
                    : <span style={{ fontSize: 10, fontWeight: 900, color: ORANGE }}>{rowIni}</span>
                  }
                </div>
              ...
              <div style={{ width: 80, textAlign: "center" }}>
                <span style={{
                  ...
                  background: "rgba(249,115,22,0.1)", borderRadius: 6,
                  padding: "2px 8px",
                }}>{p.position ?? "—"}</span>
              </div>
              {/* Edad */}
...
function StatsPage({ team, players, season }: { team: Team; players: PlayerWithStats[]; season: string }) {
  const ini = initials(team.name);
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ flex: 1 }}>
          ...
          <h2 style={{ margin: "2px 0 0", fontSize: 22, fontWeight: 900, color: "#ffffff", textTransform: "uppercase", fontStyle: "italic" }}>
            {team.name}
          </h2>
        </div>
        <div style={{ textAlign: "right" }}>
            ...
            <div key={p.id} style={{
              ...
              <div style={{ width: 220, display: "flex", alignItems: "center", gap: 8 }}>
                <div style={{
                  ...
                  {p.photoUrl
                    ? <img src={p.photoUrl} alt={p.name} style={{ width: "100%", height: "100%", objectFit: "cover" }} crossOrigin="anonymous" />
                    : <span style={{ fontSize: 9, fontWeight: 900, color: ORANGE }}>{rowIni}</span>
                  }
                </div>
                <div>
...
function SistemasPage({ team, sistemas, season, pageNum }: {
  team: Team; sistemas: MediaItem[]; season: string; pageNum: number;
}) {
  const ini = initials(team.name);
          ...
          <div key={s.id} style={{
            ...
            <div style={{
              ...
              <span style={{
                ...
                textTransform: "uppercase", letterSpacing: "0.05em", fontStyle: "italic",
              }}>
                {s.title || "Sistema"}
              </span>
            </div>
            ...
            <div style={{ display: "flex", gap: 0 }}>
              ...
              {s.description && (
                <div style={{
                  padding: "16px 20px",
                  flex: s.url ? "0 0 280px" : 1,
                  borderRight: s.url ? "1px solid #f3f4f6" : "none",
                }}>
                  <p style={{ margin: 0, fontSize: 12, color: "#4b5563", lineHeight: 1.7, whiteSpace: "pre-wrap" }}>
                    {s.description}
                  </p>
                </div>
              )}
              ...
              {/* Image */}
              {s.url && s.url.match(/\.(png|jpg|jpeg|gif|webp|svg)$/i) && (
                <div style={{ flex: 1, maxHeight: 200, overflow: "hidden" }}>
                  <img
                    src={s.url} alt={s.title ?? "Sistema"}
...
export function TeamReportExportButton({ team, season = "2025/26" }: { team: Team; season?: string }) {
  ...
  const fotos    = allMedia.filter((m) => m.category === "photo");

  // Refs for each A4 page
  const coverRef     = useRef<HTMLDivElement | null>(null);
  const plantillaRef = useRef<HTMLDivElement | null>(null);
  const statsRef     = useRef<HTMLDivElement | null>(null);
  // sistema pages — up to 3 sistemas per page
  ...
  // Pre-fetched stats (hook-in-loop workaround: fetch via imperative calls)
  const [statsMap, setStatsMap] = useState<Record<number, PlayerStats | null>>({});
  useEffect(() => {
    if (players.length === 0) return;
    Promise.all(players.map(async (p) => ({ id: p.id, stats: await fetchPlayerStats(p.id) })))
      .then((results) => {
    ...
    <>
      ...
      <div style={{ position: "fixed", left: -9999, top: 0, zIndex: -1, pointerEvents: "none" }}>
        ...
        <div ref={coverRef} style={{ width: W, height: H }}>
          <CoverPage team={team} season={season} />
        </div>

        {/* Page 2 — Plantilla */}
        <div ref={plantillaRef} style={{ width: W, height: H }}>
          <PlantillaPage team={team} players={playersWithStats} season={season} />
        </div>

        {/* Page 3 — Estadísticas */}
        <div ref={statsRef} style={{ width: W, height: H }}>
          <StatsPage team={team} players={playersWithStats} season={season} />
        </div>
        ...
        {sistemasChunks.map((chunk, idx) => (
          <div
            key={idx}
            ref={(el) => { sisRefs.current[idx] = el; }}
            style={{ width: W, height: H }}
          >
            <SistemasPage team={team} sistemas={chunk} season={season} pageNum={4 + idx} />
          </div>
        ))}
      </div>
    </>
